...
async def admin_groups(hsorek):
    hsorekgroups = []
    async for dialog in hsorek.iter_dialogs():
...
async def extract_time(hsorek, time_val):
    if any(time_val.endswith(unit) for unit in ("s", "m", "h", "d", "w")):
        unit = time_val[-1]
        time_num = time_val[:-1]  # type: str
        if not time_num.isdigit():
            await hsorek.edit("**- هذا الرقم غير صحيح  !**")
            return None
        if unit == "s":
            ...
            # how even...?
            await hsorek.edit(
                f"__خطأ في اختيار الوقت يرجى كتابه s,  m , h , d or w لكن تم اخذ:__ {time_val[-1]}"
            )
            return None
        return bantime
    await hsorek.edit(
