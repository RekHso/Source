...
class SourceBroadcast(BASE):
    __tablename__ = "Sourcebroadcast"
    ...
    def __repr__(self):
        return "<Source Broadcast channels '%s' for %s>" % (
            self.group_id,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, SourceBroadcast)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


SourceBroadcast.__table__.create(checkfirst=True)

SourceBROADCAST_INSERTION_LOCK = threading.RLock()
...
def add_to_broadcastlist(keywoard, group_id):
    with SourceBROADCAST_INSERTION_LOCK:
        broadcast_group = SourceBroadcast(keywoard, str(group_id))
...
def rm_from_broadcastlist(keywoard, group_id):
    with SourceBROADCAST_INSERTION_LOCK:
        broadcast_group = SESSION.query(SourceBroadcast).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_broadcastlist(keywoard):
    with SourceBROADCAST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(SourceBroadcast.keywoard)
            .filter(SourceBroadcast.keywoard == keywoard)
            .delete()
        )
...
def get_broadcastlist_chats():
    try:
        chats = SESSION.query(SourceBroadcast.keywoard).distinct().all()
...
def num_broadcastlist():
    try:
        return SESSION.query(SourceBroadcast).count()
    finally:
        SESSION.close()
...
def num_broadcastlist_chats():
    try:
        return SESSION.query(func.count(distinct(SourceBroadcast.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_broadcastlists():
    try:
        ...
        all_groups = SESSION.query(SourceBroadcast).all()
        for x in all_groups:
