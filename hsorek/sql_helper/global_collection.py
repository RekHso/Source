...
class source_GlobalCollection(BASE):
    __tablename__ = "source_globalcollection"
    ...
    def __repr__(self):
        return "<source Global Collection lists '%s' for %s>" % (
            self.contents,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, source_GlobalCollection)
            and self.keywoard == other.keywoard
            and self.contents == other.contents
        )


source_GlobalCollection.__table__.create(checkfirst=True)

source_GLOBALCOLLECTION = threading.RLock()
...
def add_to_collectionlist(keywoard, contents):
    with source_GLOBALCOLLECTION:
        keyword_items = source_GlobalCollection(keywoard, tuple(contents))
...
def rm_from_collectionlist(keywoard, contents):
    with source_GLOBALCOLLECTION:
        keyword_items = SESSION.query(source_GlobalCollection).get(
...
def del_keyword_collectionlist(keywoard):
    with source_GLOBALCOLLECTION:
        keyword_items = (
            SESSION.query(source_GlobalCollection.keywoard)
            .filter(source_GlobalCollection.keywoard == keywoard)
            .delete()
        )
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(source_GlobalCollection.keywoard).distinct().all()
...
def num_collectionlist():
    try:
        return SESSION.query(source_GlobalCollection).count()
    finally:
        SESSION.close()
...
def num_collectionlist_items():
    try:
        return SESSION.query(
            func.count(distinct(source_GlobalCollection.keywoard))
        ).scalar()
    finally:
        SESSION.close()
...
def __load_item_collectionlists():
    try:
        ...
        all_groups = SESSION.query(source_GlobalCollection).all()
        for x in all_groups:
