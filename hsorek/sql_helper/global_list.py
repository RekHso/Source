...
class sourceGloballist(BASE):
    __tablename__ = "sourceglobal_list"
    ...
    def __repr__(self):
        return "<source global values '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, sourceGloballist)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


sourceGloballist.__table__.create(checkfirst=True)

sourceGLOBALLIST_INSERTION_LOCK = threading.RLock()
...
def add_to_list(keywoard, group_id):
    with sourceGLOBALLIST_INSERTION_LOCK:
        broadcast_group = sourceGloballist(keywoard, str(group_id))
...
def rm_from_list(keywoard, group_id):
    with sourceGLOBALLIST_INSERTION_LOCK:
        broadcast_group = SESSION.query(sourceGloballist).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_list(keywoard):
    with sourceGLOBALLIST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(sourceGloballist.keywoard)
            .filter(sourceGloballist.keywoard == keywoard)
            .delete()
        )
...
def get_list_keywords():
    try:
        chats = SESSION.query(sourceGloballist.keywoard).distinct().all()
...
def num_list():
    try:
        return SESSION.query(sourceGloballist).count()
    finally:
        SESSION.close()
...
def num_list_keywords():
    try:
        return SESSION.query(func.count(distinct(sourceGloballist.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_lists():
    try:
        ...
        all_groups = SESSION.query(sourceGloballist).all()
        for x in all_groups:
