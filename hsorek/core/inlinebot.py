...
def main_menu():
    text = f"**▾∮ مرحبا عزيزي {mention}**\n**▾اليكَ قائمة بازرار مضمنة لاوامر ↫**⍣ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ⍣**\n[♥](tg://settings/)\n\n"
    buttons = [
...
async def inline_handler(event):
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**hsorek"):
            buttons = [
                (
                    Button.inline("Stats", data="stats"),
                    Button.url("DEV", "tg://settings/"),
                )
            ]
            ...
            builder = event.builder
            troll = os.path.join("./hsorek", "troll.txt")
            try:
            ...
            try:
                ...
                try:
                    ...
                    if u.username:
                        ...
                        HPPHH = f"[{u.first_name}](tg://user?id={u.id})"
                    u = int(u.id)
                except ValueError:
                    HPPHH = f"[user](tg://user?id={u})"
            except ValueError:
                # if u is username
                try:
            ...
            result = builder.article(
                ...
                text=f"🔒 هذه همسة سرية الى {HPPHH}, هو فقط من يستطيع رؤيتها",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            if jsondata:
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./hsorek", "secrets.txt")
            try:
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./hsorek", "hide.txt")
            try:
            ...
            result = builder.article(
                title="hsorek Help™",
                description="**▾∮ قائمة التعليمات الخاصة بسورسك **",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
...
@hsorek.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@hsorek.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@hsorek.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@hsorek.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
